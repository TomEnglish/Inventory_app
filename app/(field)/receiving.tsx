...
export default function ReceivingScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!isOnline) {
        // Queue for later sync (photos excluded — they require network)
        await addToQueue({
          type: 'receiving',
          payload: {
            ...
            photos: [], // Photos can't be queued offline
        ...
        // Look up or create the QR code
        const qr = await lookupOrCreateQRCode(store.qrCodeValue);

        // Submit the receiving record
